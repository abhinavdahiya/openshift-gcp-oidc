...
resource "google_service_account_iam_member" "assume" {
  ...
  member             = "principal://iam.googleapis.com/projects/${data.google_project.google_project.number}/locations/global/workloadIdentityPools/${var.identity_pool}}/subject/${var.iam_namespace}.svc.id.openshift[${each.value}]"
}
